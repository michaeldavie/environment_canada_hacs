...
async def init_integration(hass: HomeAssistant, ec_data) -> MockConfigEntry:
    ...
    with (
        patch(
            "custom_components.environment_canada.ECWeather",
            return_value=weather_mock,
        ),
        patch(
            "custom_components.environment_canada.ECAirQuality",
            return_value=mock_ec(),
        ),
        patch(
            "custom_components.environment_canada.ECMap",
            return_value=radar_mock,
        ),
        patch(
            "custom_components.environment_canada.config_flow.ECWeather",
            return_value=weather_mock,
        ),
    ):
